...
type MemoryRepo interface {
	...
	// DebateMemory:dialog:{DebateMemoryTag}
	...
	// DebateMemory:base:{DebateMemoryTag}
...
// 原子获取并加一 NextDebateTag 值，返回加一后的数值
...
// 添加 tag 这个元素到 ActiveDebateMemoryTags set 中
...
// 从 ActiveDebateMemoryTags set 中删除 tag 这个元素
func (r *MemoryRepoImpl) RemoveActiveDetabeMemoryTag(ctx context.Context, tag int) error {
	return r.rdb.SRem(ctx, "ActiveDebateMemoryTags", tag).Err()
}

// 获取 ActiveDebateMemoryTags 这个 set 中所有的 tag
...
func (r *MemoryRepoImpl) GetDebateMemoryBase(ctx context.Context, tag int) (*model.DebateMemory, error) {
	...
	// 结构体中存有 tag
	return &debateMemory, err
}

// 设置某个具体的 DebateMemory 的固定部分
